...
function CreatePDF() {
  ...
  const generatePDFFromStream = useCallback(async () => {
    const urlParams = new URLSearchParams(window.location.search);
    const paramValue = urlParams.get("id");
    if (pathname === "/create-quotation" && paramValue !== null) {
      await axiosInstance
        .get(`/quotation/downloadPDF?id=${paramValue}`)
        .then((res) => {
          if (res.data.status === true) {
            ...
            async function extractPDFPage() {
              try {
                ...
                const pdfSrcDoc = await PDFDocument.load(bufferData);
                const pdfNewDoc = await PDFDocument.create();
                const pages = pdfSrcDoc.getPages();
                for (const [pageIndex, page] of pages.entries()) {
                  const [copiedPage] = await pdfNewDoc.copyPages(pdfSrcDoc, [
                    pageIndex,
                  ]);
